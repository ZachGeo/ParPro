...
void RamerDouglasPeucker(const vector<Point> &pointList, double epsilon, vector<Point> &out)
{
	...
	// Δημιουργία έργου(task) για την επεξεργασία της 1ης πολυγραμμής. Το block του κώδικα αυτού εκτελείται από το νήμα που το συναντά.
	#pragma omp task firstprivate(firstLine, epsilon) shared(recResults1)
	{
		RamerDouglasPeucker(firstLine, epsilon, recResults1);
	}	
	
	// Δημιουργία έργου(task) για την επεξεργασία της 2ης πολυγραμμής. Το block του κώδικα αυτού εκτελείται από το νήμα που το συναντά(διαφορετικό νήμα από αυτό που εκτελεί το 1ο έργο).
	#pragma omp task firstprivate(lastLine, epsilon) shared(recResults2)
	{
		RamerDouglasPeucker(lastLine,  epsilon, recResults2);
	}
		
		// Μόλις το νήμα που δημιούργησε άλλα έργα συναντήση αυτήν την εντολή, τότε πρέπει να περιμένει την ολοκλήρωση αυτών.
		#pragma omp taskwait		
...
#pragma omp parallel 
{	
	// Δυναμικός τρόπος ανάθεσης πολυγραμμών στα νήματα. Το chunk δεν δηλώνετε, με αποτέλεσμα οι επαναλήψεις να χωρίζονται σε κομμάτια που είναι περίπου ίσα σε μέγεθος. Επίσης, το πρότυπο OpenMP απαιτεί ο βρόχος να είναι σε "κανονική μορφή". Οπότε στον έλεγχο των επαναλήψεων αλλάξαμε το σύμβολο "!=" σε "<".
	#pragma omp for ordered schedule(dynamic)
	for (vector< vector<Point> >::const_iterator it1 = AllPolylines.begin(); it1 < AllPolylines.end(); it1++) {
			
			// Κάθε νήμα καλεί τον δικό του δείκτη στο αντίστοιχο vector.
			vector<Point>			polyline, simplified_polyline;
			polyline = (*it1);
			simplified_polyline.clear();
		
			RamerDouglasPeucker(polyline, epsilon, simplified_polyline);
			// Τα διαφορετικά νήματα μόλις συναντήσουν την παρακάτω εντολή, τότε τα νήματα εκτελούν, ένα προς ένα (σειριακά) την εντολή η οποία προσθέτει ένα καινούργιο στοιχείο στο συγκεκριμένο vector.
			#pragma omp ordered			  
			SimplifiedAllPolylines.push_back(simplified_polyline);
		
	}
}
// Τέλος Παράλληλης Περιοχής.
